...
(function () {
  ...
  const JAMO = {
    ...
    // 아래 섞임모음 7개는 JAMO_MIX 로 나눠 앉히므로 음절 조립에는 쓰지 않는다
    // (한 상자에 통째로 넣으면 초성과 겹친다). 낱자 모양 참고용으로만 남긴다.
  ...
  const KO_MIX = 'ㅘㅙㅚㅝㅞㅟㅢ';     // 섞임모음

  // 섞임모음은 '가로 부분 + 세로 부분'으로 나눠 따로 앉힌다.
  // 한 상자에 통째로 넣으면 가로 부분(ㅗ/ㅜ/ㅡ)의 막대가 초성 자리를 관통해
  // 휠·활·쉘·화처럼 글자가 뭉개진다. 실제 한글도 이렇게 짜인다 —
  // 왼쪽 위에 초성, 그 아래 가로 부분, 오른쪽에 세로 부분.
  // 획 순서는 초성 → 가로 부분 → 세로 부분 → 종성 (한글 획순과 같다).
  const JAMO_MIX = {
    ㅘ: ['ㅗ', 'ㅏ'], ㅙ: ['ㅗ', 'ㅐ'], ㅚ: ['ㅗ', 'ㅣ'],
    ㅝ: ['ㅜ', 'ㅓ'], ㅞ: ['ㅜ', 'ㅔ'], ㅟ: ['ㅜ', 'ㅣ'],
    ㅢ: ['ㅡ', 'ㅣ'],
  ...
  // C·Cb(섞임모음)만 jung 이 둘로 나뉜다 — jungH(가로 부분), jungV(세로 부분)
  const KO_BOX = {
    ...
    C: { cho: [3, 3, 54, 41], jungH: [4, 44, 60, 93], jungV: [58, 4, 97, 96] },
    Cb: {
      cho: [3, 2, 55, 31], jungH: [4, 34, 58, 60],
      jungV: [56, 2, 95, 63], jong: [14, 64, 86, 97],
    },
  };
  ...
  function strokesFor(ch) {
    ...
    let out = jamoStrokes(cho, box.cho);
    if (type === 'C') {
      const pair = JAMO_MIX[jung];
      out = out
        .concat(jamoStrokes(pair[0], box.jungH))
        .concat(jamoStrokes(pair[1], box.jungV));
    } else {
      out = out.concat(jamoStrokes(jung, box.jung));
    }
  ...
  const W = {
    ...
    penW() {
      ...
      // 획이 많을수록 칸이 좁아지니 더 얇게 — 휠·활·뿔처럼 겹받침·쌍자음이 있는
      // 글자는 굵은 획으로 그리면 서로 붙어 덩어리가 된다.
      if (this.char < '가' || this.char > '힣') return 15;
